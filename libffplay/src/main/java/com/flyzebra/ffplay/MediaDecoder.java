...
public class MediaDecoder {
    ...
    public MediaDecoder(ICallBack iCallBack, Surface surface, int widht, int height, int fps, byte[] sps, byte[] pps) {
        ...
        this.fps = fps;
        is_qsv = PropUtil.get(Prop.WEBCAM_H264QSV, "false").equals("true");
        String url = PropUtil.get(Prop.WEBCAM_URL, "");
        ...
        try {
            ...
            if (outIndex >= 0) {
                ...
                if (surface == null) {
                    ...
                    buffer.limit(info.offset + glDataSize);
                    buffer.get(glData, 0, glDataSize);
